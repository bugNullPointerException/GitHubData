var identifyAnalysis = {
	dateFrom : "",
	dateTo : "",
	dateFromTo : [],

	setDateFrom : function(date) {
		this.dateFrom = date;
	},
	getDateFrom : function() {
		return this.dateFrom;
	},
	setDateTo : function(date) {
		this.dateTo = date;
	},
	getDateTo : function() {
		return this.dateTo;
	},
	setDateFromTo : function(date) {
		this.dateFromTo = date;
	},
	getDateFromTo : function() {
		return this.dateFromTo;
	},

	// 选择开始时间后触发
	selectTimeFrom : function() {
		// 对开始时间重新set一个值（去除默认时间）
		identifyAnalysis.setDateFrom($dp.cal.getDateStr("yyyy-MM-dd"));
		// 重新生成时间时间数组
		identifyAnalysis.setDateFromTo(getSomeDates(identifyAnalysis.getDateFrom(), identifyAnalysis.getDateTo()));
		// 回调加载数据函数
		identifyAnalysis.loadData(identifyAnalysis.getDateFrom(), identifyAnalysis.getDateTo());
	},
	// 选择结束时间后触发
	selectTimeTo : function() {
		// 对结束时间重新set一个值（去除默认时间）
		identifyAnalysis.setDateTo($dp.cal.getDateStr("yyyy-MM-dd"));
		// 重新生成时间时间数组
		identifyAnalysis.setDateFromTo(getSomeDates(identifyAnalysis.getDateFrom(), identifyAnalysis.getDateTo()));
		// 回调加载数据函数
		identifyAnalysis.loadData(identifyAnalysis.getDateFrom(), identifyAnalysis.getDateTo());
	},

	// 初始化
	init : function() {
		identifyAnalysis.initParms();
	},
	// 初始化参数
	initParms : function() {
		$("#chooseTimeFrom").attr("placeholder",getWeekAgoDate());
		$("#chooseTimeTo").attr("placeholder",getNowDate());
		// 获取当前时间
		identifyAnalysis.setDateFrom(getWeekAgoDate());
		// 获取一周前的时间
		identifyAnalysis.setDateTo(getNowDate());
		// 获取默认的七天时间
		identifyAnalysis.setDateFromTo(getSomeDates(identifyAnalysis.getDateFrom(), identifyAnalysis.getDateTo()));
//		$("#chooseTimeFrom").attr("placeholder","2017-07-13");
//		$("#chooseTimeTo").attr("placeholder","2017-07-20");
//		identifyAnalysis.setDateFrom("2017-07-13");
//		identifyAnalysis.setDateTo("2017-07-20");
//		identifyAnalysis.setDateFromTo(getSomeDates(identifyAnalysis.getDateFrom(), identifyAnalysis.getDateTo()));
		// 回调加载数据函数
		identifyAnalysis.loadData(identifyAnalysis.getDateFrom(), identifyAnalysis.getDateTo());
	},

	// 加载数据
	loadData : function(dataFrom, dataTo) {
		identifyAnalysis.getAgencyCustomerAnalysis("agencyCustomerAnalysis", dataFrom, dataTo);
	},

	// 加载代购客户分析
	getAgencyCustomerAnalysis : function(id,dataFrom, dataTo) {
		var option = chartTemplate.createNewLine();
		var myCharts = echarts.init(document.getElementById(id));
		var legend = ["全部用户", "普通用户","代购用户"];
		option.title.setText("代购客户分析");
		option.series[0].setName("全部用户");
		option.series[1].setName("普通用户");
		option.series[2].setName("代购用户");
		option.legend.setData(legend);
		option.removeSerie(3,2);
		option.toolbox.feature.saveAsImage.setName(option.title.getText() + getNowFormatDate());
		
		var seriesData0 = [];
		var seriesData1 = [];
		var seriesData2 = [];
		var seriesType0 = [];
		var seriesType1 = [];
		var seriesType2 = [];
		var xAxisData = identifyAnalysis.getDateFromTo();
		//调用ajax前对开始时间和结束时间做校验
		if(dataFrom > dataTo){
			alert("开始时间不能大于结束时间");
			return;
		}else{
			var content = {
				"startDate":dataFrom,
				"endDate":dataTo
			};
		 	var url = ctx + '/agencyCustomerAnalysis/getNhAgencyCustomerAnalysis';
		 	kingpoint.postData(url,content,function(data){
		 		if(data!=null && data.length!=0){
		 			//对数据进行分类
					for(var i=0; i<data.length; i++){
						if(data[i].customerType == 0){
							seriesType0.push(data[i]);
						}else if(data[i].customerType == 1){
							seriesType1.push(data[i]);
						}else if(data[i].customerType == 2){
							seriesType2.push(data[i]);
						}
					}
			 		//找到x轴对应的数据 
			 		seriesData0 = commonUtil.calculateValue(seriesType0,xAxisData);
			 		seriesData1 = commonUtil.calculateValue(seriesType1,xAxisData);
			 		seriesData2 = commonUtil.calculateValue(seriesType2,xAxisData);
			 	}
		 		option.xAxis.setData(xAxisData);
				option.series[0].setData(seriesData0);
				option.series[1].setData(seriesData1);
				option.series[2].setData(seriesData2);
				myCharts.setOption(option);
		 	});
		}
	 	option.xAxis.setData(xAxisData);
		option.series[0].setData(seriesData0);
		option.series[1].setData(seriesData1);
		option.series[2].setData(seriesData2);
	 	myCharts.setOption(option);
	}
};

$(function(){
	identifyAnalysis.init();
});