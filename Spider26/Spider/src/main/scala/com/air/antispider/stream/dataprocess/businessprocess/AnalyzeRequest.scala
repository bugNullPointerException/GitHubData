...
object AnalyzeRequest {
  ...
  /**
    * 不同类型数据，根据不同解析规则进行解析
    */
  def analyzeQueryRequest(requestTypeLabel: RequestType, //上面打的标签
                          requestMethod: String, //get/post
                          contentType: String,
                          request: String,
                          requestBody: String,
                          travelType: TravelTypeEnum, //往返类型
                          analyzeRules: List[AnalyzeRule] //从数据库查询出来的解析规则
                         ): Option[QueryRequestData] = {
